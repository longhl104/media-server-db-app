...
def single_song(song_id):
    ...
    songlocation = None
    songlocation = database.get_song_location(song_id)

    songartist = None
    songartist = database.get_song_artist(song_id)
...
def single_podcastep(media_id):
    ...
    page['title'] = 'List Podcast Episodes' # Add the title
...
#   Add Podcast
#####################################################
@app.route('/add/podcast', methods=['POST','GET'])
def add_podcast():
    """
    Add a new podcast
    """
    ...
    if bool(database.is_superuser([user_details['username']])) == False :
        ...
        if (database.is_superuser(user_details['username'])):
            page['bar'] = False
            flash("You do not have access to this feature.")

        # Check your incoming parameters
        if(request.method == 'POST'):
            #print (database.is_superuser(user_details['username']))

            # verify that the values are available:
            if ('podcast_title' not in request.form):
                newdict['podcast_title'] = 'Empty Podcast Value'
            else:
                newdict['podcast_title'] = request.form['podcast_title']
                print("We have a value: ",newdict['podcast_title'])

            if ('podcast_uri' not in request.form):
                newdict['podcast_uri'] = 'Empty URI Value'
            else:
                newdict['podcast_uri'] = request.form['podcast_uri']
                print ("We have a value: ",newdict['podcast_uri'])

            if ('podcast_last_updated' not in request.form):
                newdict['podcast_last_updated'] = '2019-01-01'
            else:
                newdict['podcast_last_updated'] = request.form['podcast_last_updated']
                print ("We have a value: ",newdict['podcast_last_updated'])
            ...
            podcasts = database.add_podcast_to_db(newdict['podcast_title'],newdict['podcast_uri'],newdict['podcast_last_updated'],newdict['description'])


            max_podcast_id = database.get_last_podcast()[0]['podcast_id']
            print(podcasts)
            if podcasts is not None:
                max_podcast_id = podcasts[0]

            # ideally this would redirect to your newly added artist
            return single_podcast(max_podcast_id)
        else:
            return render_template('createitems/createpodcast.html',
                            session=session,
                            page=page,
                            user=user_details)
...
def add_song():
    ...
    if bool(database.is_superuser([user_details['username']])) == False :
        ...
        if (request.method == 'POST'):
            ...
            newdict['description'],newdict['song_title'],
            newdict['song_length'],newdict['genre'],newdict['artistId'])
...
def add_artist():
    ...
    if bool(database.is_superuser([user_details['username']])) == False :
        ...
        if(request.method == 'POST'):
            ...
            #forward to the database to manage insert
            artists = database.add_artist_to_db(newdict['artist_name'],newdict['description'])
...
#   Add TV Show
#####################################################
@app.route('/add/tvshow', methods=['POST','GET'])
def add_tvshow():
    """
    Add a new TV show
    """
    ...
    if bool(database.is_superuser([user_details['username']])) == False :
        ...
        if(request.method == 'POST'):
            ...
            #forward to the database to manage insert
            tvshows = database.add_tvshow_to_db(newdict['tvshow_title'],newdict['description'])
