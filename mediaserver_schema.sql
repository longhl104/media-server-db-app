...
$BODY$
    ...
    SELECT (max(artist_id) + 1) as artist_id FROM mediaserver.artist;
$BODY$
LANGUAGE sql;
...
create or replace function mediaserver.addtvshow(
    title varchar(250),
	tvshowdescription text)
RETURNS int AS
$BODY$
	WITH ins1 AS (
        INSERT INTO mediaserver.tvshow (tvshow_title)
        VALUES (title)
        RETURNING tvshow_id
    )
    ,ins2 AS (
        INSERT INTO mediaserver.metadata (md_type_id,md_value)
        SELECT md_type_id, tvshowdescription
        FROM mediaserver.MetaDataType where md_type_name = 'description'
        RETURNING md_id
        )
    ,ins3 AS (
        INSERT INTO mediaserver.TVShowMetaData(tvshow_id, md_id)
        SELECT tvshow_id, md_id FROM ins1, ins2
        )

        SELECT (max(tvshow_id)+1) as tvshow_id FROM mediaserver.tvshow;
$BODY$
LANGUAGE sql;

create or replace function mediaserver.addPodcast(
	podcast_title varchar(250),
    podcast_uri text,
    podcast_last_updated Date,
...
$BODY$
	WITH ins1 AS (
        INSERT INTO mediaserver.Podcast (podcast_title, podcast_uri, podcast_last_updated)
        VALUES (podcast_title   ,podcast_uri,podcast_last_updated)
        RETURNING podcast_id
        ),
        ins2 AS (
        INSERT INTO mediaserver.metadata (md_type_id,md_value)
        SELECT md_type_id, podcastdescription
        FROM mediaserver.MetaDataType where md_type_name = 'description'
        RETURNING md_id
        ),
        ins3 AS (
        INSERT INTO mediaserver.PodcastMetaData (podcast_id, md_id)
        SELECT podcast_id, md_id FROM ins1, ins2)

        SELECT (max(podcast_id) + 1) as podcast_id FROM mediaserver.podcast;
$BODY$
LANGUAGE sql;
